...
export default {
  ...
  watch: {
    sku: {
      ...
      immediate: true,
      handler (sku) {
        this.fetchLeafById(sku[this.optionValue], sku.values)
      }
    }
  },
  ...
  methods: {
    ...
    fetchLeafById (id, values) {
      if (!id) return
      if (values == undefined) {
        this.skuTree.forEach(item => {
          if (parseInt(id) === parseInt(item.id)) {
            this.skuOptions = item.values;
          }
        });
      } else {
        this.ease.onFetchSku(id).then(skuOptions => {
          this.id = id;
          this.skuOptions = values;
        })
      }
    },
    ...
    handleUploadSuccess (response, file, fileList, id) {
      ...
      sku.leaf.forEach(item => {
        if (item[optionValue] === id) {
          //item.img_url = this.myConfig.img_url + response.result.filepath
          item.img_url = response.result.filepath
        }
      })
  ...
  created () {
    ...
    sku[optionValue] && this.fetchLeafById(sku[optionValue], sku.values)
  }
}
</script>
