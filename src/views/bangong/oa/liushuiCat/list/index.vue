...
<template>
  <div>
    ...
    <section class="flex-items marginb20 marginr16 search">
      ...
      <el-select size="small" v-model="form.type" clearable placeholder="请选择科目类型">
        <el-option v-for="item in goodsOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
      </el-select>

      <el-select size="small" v-model="form.is_sys" clearable placeholder="请选择系统科目">
        <el-option v-for="item in xtOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
      </el-select>
    ...
    <section class="flex-items marginb20">
      <router-link :to="'/manage/liushuiCat/add'"><el-button size="small" type="primary">添加科目</el-button></router-link>
    </section>
    <!-- 列表 -->
