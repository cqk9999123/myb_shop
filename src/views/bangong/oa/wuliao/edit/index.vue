  ...
  <section class="add">
    ...
    >
      ...
      <el-form-item label="报警数量:" prop="alarm_num">
        <el-input v-model="form.alarm_num" placeholder="请输入低于多少开始警报" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <div class="paddingl">
  ...
  methods: {
    ...
    async getInfo () {
      ...
      this.form = {
        ...
        pics: result.pics, // 物料图片
        title: result.title, // 物料名称
        spec: result.spec, // 规格
        barcode: result.barcode, // 条码
        unit: result.unit, // 单位
        source: result.source, // 进货渠道
        stock_alarm: result.stock_alarm, // 库存警报
        alarm_num: result.alarm_num, // 低于多少开始警报
      };
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          try{
            ...
            data.url = "/Api/Admin/Wuliao/edit";
            let res = await this.request(data);
