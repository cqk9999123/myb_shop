...
<template>
  <div class="list">
    <el-table :data="list">
      ...
      <el-table-column width="120" label="推荐位图片">
        <template slot-scope="item">
          <imgs :img="item.row.pic" :width="60" :height="60"/>
        </template>
      </el-table-column>
      <el-table-column prop="title" label="推荐位标题"></el-table-column>
      <el-table-column prop="" label="推荐类型">
        <template slot-scope="item">
          {{utils.getType('goods_rec_type',item.row.type)}}
        </template>
      </el-table-column>
      <el-table-column prop="uuid" label="推荐位标识"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作" align="center" width="130">
        <template slot-scope="item">
          <router-link :to="'/mall/goodsRec/edit?id='+item.row.id">编辑</router-link>
  ...
  methods: {
    ...
    async statusChange(row) {
      ...
      let param = {
        ...
        url:"/Api/Admin/goodsRec/editVal",
      };
