...
export default {
  ...
  mounted() {
    this.getTree(this.$route.query.id);
  },
  methods: {
    //获取分类列表
    async getTree(id) {
      let param = {
        ...
        url:"/Api/Admin/ShopSlideCat/getList",
        type:'get'
      };
    ...
    async getInfo(id){
      let param = {
        ...
        url:"/Api/Admin/ShopSlide/getInfo",
      }
      let res = await this.request(param);
      let d = res.result;
      this.form = {
      ...
      if (d.pic) this.images.push({path: d.pic});
    },
    //提交
    submit() {
